...
} window.customElements && !window.customElements.get(t) && window.customElements.define(t, o); const a = "a-tab-panel", l = document.createElement("template"); let r = 0; l.innerHTML =/* html */`
 ...
 */class v extends HTMLElement {
   ...
   */#u() {
    let t = this.#d();// Hide the tab group if there are no tabs.
   ...
   */#f() {
    let t = this.#d(), e = this.activation === p ? t.findIndex(t => t.matches(":focus")) - 1 : t.findIndex(t => t.selected) - 1;// Keep looping until we find a non-disabled tab.
   ...
   */#r = t => {
    let e; if (t.target.tagName.toLowerCase() === c// Ignore any key presses that have a modifier.
      && !t.altKey// Don’t handle modifier shortcuts typically used by assistive technology.
    ) {
      switch (t.code) {
        case m.LEFT: case m.UP: e = this.#f(), this.activation === p ? e.focus() : this.selectTab(e); break; case m.RIGHT: case m.DOWN: e = this.#_(), this.activation === p ? e.focus() : this.selectTab(e); break; case m.HOME: e = this.#v(), this.activation === p ? e.focus() : this.selectTab(e); break; case m.END: e = this.#w(), this.activation === p ? e.focus() : this.selectTab(e); break; case m.ENTER: case m.SPACE: e = t.target, this.selectTab(e); break; default:// Any other key press is ignored and passed back to the browser.
          return
   ...
   */selectTab(t) {
    !t || t.disabled || t.selected || (this.#p(t),// Queue a microtask to ensure that the tab is focused on the next tick.
      setTimeout(() => t.focus(), 0), this.dispatchEvent(new CustomEvent(`${c}-select`, { bubbles: !0, composed: !0, detail: { tabId: t.id } })))
  }
} window.customElements && !window.customElements.get(n) && window.customElements.define(n, v);//# sourceMappingURL=a-tab-group.js.map
